if (!config) {
  ...
  config.filename = function () {
    var tmp = document.title || document.location.toString();
    var host = (document.location.host !== undefined ? document.location.host : "screenshot").replace("www.", '');
    var title = tmp ? tmp.replace(/[:*?"<>|\r\n]/g, '').replace(/[\t \/]+/g, '').trim().replace(/ /g, '-') : "page";
    /*  */
    return host + '-' + title;
  };
